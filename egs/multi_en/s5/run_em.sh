...
cpath=/media/second_drive/corpora/

# preset paths
case $(hostname) in
  Goliath)
    ami=$cpath/ami
    fisher="$cpath/LDC2004T19 $cpath/LDC2005T19 \
      $cpath/LDC2004S13 $cpath/LDC2005S13"
    librispeech=$cpath/librispeech
    swbd=$cpath/LDC97S62
    tedlium2=$cpath/TEDLIUM_release2
    wsj0=$cpath/LDC93S6B/csr_1_senn
    wsj1=$cpath/LDC94S13B/csr_senn
    eval2000="$cpath/LDC2002S09/hub5e_00 $cpath/LDC2002T43/2000_hub5_eng_eval_tr"
...
if [ $stage -le 7 ]; then
  ...
  corpora="librispeech_100 librispeech_360 librispeech_500 swbd tedlium wsj"
  for c in $corpora; do
    data=data/$c/train
...
if [ $stage -le 17 ]; then
  ...
  for e in eval2000 librispeech; do
...
if [ $stage -le 20 ]; then
  ...
  #stage 7 is ivec extractor
  local/nnet3/run_tdnn.sh --stage 10 --train-stage 840 --multi $multi
fi
...
if [ $stage -le 21 ]; then
  ...
  #  exp/$multi/tri5 exp/$multi/tri5_ali || exit 1;
  #stage 7 is ivec extractor
  local/nnet3/run_tdnn_noivec.sh --stage 11 --multi $multi
fi
...
if [ $stage -le 22 ]; then
  ...
  #  exp/$multi/tri5 exp/$multi/tri5_ali || exit 1;
  #stage 7 is ivec extractor
  local/chain/run_tdnn_lstm.sh --stage 13 --train-stage 2722 --multi $multi 
# tdnn + lstm rvb2 (chain), ivector max1, split 0.6 0.3 0.1
if [ $stage -le 30 ]; then
  local/chain/multi_condition/run_tdnn_lstm_7k_em.sh --stage 17 --multi $multi
fi
...
if [ $stage -le 23 ]; then
  ...
  #  exp/$multi/tri5 exp/$multi/tri5_ali || exit 1;
  #stage 7 is ivec extractor
  local/chain/run_tdnn.sh --stage 15 --multi $multi 
fi

# tdnn rvb1 (chain)
if [ $stage -le 24 ]; then
  local/chain/multi_condition/run_tdnn_7k_em.sh --stage 17 --multi $multi
fi
...
# tdnn rvb2 (chain, large)
if [ $stage -le 26 ]; then
  local/chain/multi_condition/run_tdnn_7k_em2.sh --stage 12 --multi $multi
fi

# tdnn rvb2 (chain), ivector max1
if [ $stage -le 27 ]; then
  local/chain/multi_condition/run_tdnn_7k_em.sh --stage 13 --multi $multi
fi

# tdnn rvb2 (chain), ivector max1, split 0.6 0.3 0.1
if [ $stage -le 28 ]; then
  local/chain/multi_condition/run_tdnn_7k_em.sh --stage 17 --multi $multi
fi
...
# tdnn rvb2 (chain), ivector max1, split 0.6 0.3 0.1
if [ $stage -le 29 ]; then
  local/chain/multi_condition/run_tdnn_7k_em.sh --stage 12 --train-stage 3339 --multi $multi
fi

# tdnn + lstm rvb2 (chain), ivector max1, split  sim + rir + noises
if [ $stage -le 31 ]; then
  local/chain/multi_condition/run_tdnn_7k_em.sh --stage 17 --train-stage 2717 --multi $multi
fi
