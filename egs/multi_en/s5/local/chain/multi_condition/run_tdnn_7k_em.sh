...
get_egs_stage=-10
speed_perturb=false
dir=exp/$multi/chain/tdnn_7k_max1_rn_d800  # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_nj=30
ivector_dir=exp/$multi/nnet3_rvb_rn
...
if [ $stage -le 12 ]; then
  ...
  relu-batchnorm-layer name=tdnn1 dim=800
  relu-batchnorm-layer name=tdnn2 input=Append(-1,0,1) dim=800
  relu-batchnorm-layer name=tdnn3 dim=800
  relu-batchnorm-layer name=tdnn4 input=Append(-1,0,1) dim=800
  relu-batchnorm-layer name=tdnn5 dim=800
  relu-batchnorm-layer name=tdnn6 input=Append(-3,0,3) dim=800
  relu-batchnorm-layer name=tdnn7 input=Append(-3,0,3) dim=800
  relu-batchnorm-layer name=tdnn8 input=Append(-3,0,3) dim=800
  relu-batchnorm-layer name=tdnn9 input=Append(-3,0,3) dim=800

  ## adding the layers for chain branch
  relu-batchnorm-layer name=prefinal-chain input=tdnn9 dim=800 target-rms=0.5
  ...
  relu-batchnorm-layer name=prefinal-xent input=tdnn9 dim=800 target-rms=0.5
