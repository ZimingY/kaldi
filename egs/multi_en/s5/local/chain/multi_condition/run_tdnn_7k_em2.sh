...
get_egs_stage=-10
speed_perturb=false
dir=exp/$multi/chain/tdnn_7k_em2  # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_nj=30
ivector_dir=exp/$multi/nnet3_rvb
...
frames_per_eg=150
remove_egs=false
common_egs_dir=exp/multi_a/chain/tdnn_7k_rvb2/egs
xent_regularize=0.1
has_fisher=false
...
if [ $stage -le 12 ]; then
  ...
  relu-batchnorm-layer name=tdnn2 input=Append(-1,2) dim=1024
  relu-batchnorm-layer name=tdnn3 input=Append(-3,3) dim=1024
  relu-batchnorm-layer name=tdnn4 input=Append(-3,3) dim=1024
  relu-batchnorm-layer name=tdnn5 input=Append(-6,2) dim=1024

  ## adding the layers for chain branch
  relu-batchnorm-layer name=prefinal-chain input=tdnn5 dim=1024 target-rms=0.5
  ...
  relu-batchnorm-layer name=prefinal-xent input=tdnn5 dim=1024 target-rms=0.5
...
if $test_online_decoding && [ $stage -le 17 ]; then
  ...
  steps/online/nnet3/prepare_online_decoding.sh \
       ...
       $lang exp/$multi/nnet3_rvb/extractor $dir ${dir}_online
